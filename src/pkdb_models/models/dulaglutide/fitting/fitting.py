...
from pkdb_models.models.dulaglutide.fitting.fit_experiments import (
    f_fitexp_pharmacokinetics,
    f_fitexp_pharmacodynamics,
)
from pkdb_models.models.dulaglutide.fitting.parameters import (
    parameters_pharmacokinetics,
    parameters_pharmacodynamics,
)

from pkdb_models.models.dulaglutide import (
    DULAGLUTIDE_PATH,
    DATA_PATHS,
)
...
def get_fit_experiments(fit_subset: FitExperimentSubset, study_ids: List[str] = None):
    ...
    if fit_subset == FitExperimentSubset.PHARMACOKINETICS:
        fitexp_dict = f_fitexp_pharmacokinetics()
    elif fit_subset == FitExperimentSubset.PHARMACODYNAMICS:
...
def main() -> None:
    ...
    The script is registered as `fit_dulaglutide` command.
    """
    ...
    console.print(":wrench: FIT DULAGLUTIDE :wrench:")
    console.rule(style="white")
    ...
    if not options.output_dir:
        from pkdb_models.models.dulaglutide import RESULTS_PATH_FIT
    ...
    results: Dict[str, Tuple[OptimizationResult, OptimizationProblem]] = fit_dulaglutide(
        fit_experiments=fit_experiments,
...
if __name__ == "__main__":
    ...
    fit_dulaglutide
    fit_dulaglutide --cores=10 --runs=10 --seed=1234 --method=LSQ --strategy=ALL --subset=PHARMACOKINETICS --name=DULAGLUTIDE_LSQ_PHARMACOKINETICS
    fit_dulaglutide --cores=10 --runs=10 --seed=1234 --method=LSQ --strategy=ALL --subset=PHARMACODYNAMICS --name=DULAGLUTIDE_LSQ_PHARMACODYNAMICS
    """
    main()