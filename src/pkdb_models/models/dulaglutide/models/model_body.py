...
_m = Model(
    "dulaglutide_body",
    name="Dulaglutide body model",
    notes="""
    # Dulaglutide body model
    
    Physiological based pharmacokinetic/pharmacodynamic (PBPK/PD) model of dulaglutide.
    """
...
SUBSTANCES_BODY = {
    "dul": {
        ...
        "SCDOSE": 0,  # subcutaneous
    },
    "dm": {
        "name": "dulaglutide metabolites",
        "unit": U.mmole,
        # initial concentration
        "cinit": 0.0,  # [mmole/l]
        # Molecular weight
        "Mr": 3314.6,  # [g/mole]  # FIXME: unknown weight, mix of different metabolites (cleavage)
    },
}
...
kidney_id = "dulaglutide_kidney"
liver_id = "dulaglutide_liver"
gut_id = "dulaglutide_intestine"
...
_m.parameters.extend(
    [
        # whole body data
        # BW and HEIGHT defined in bodyweight submodel
        # Parameter(
        #     "BW",
        #     75,
        #     U.kg,
...
_m.reactions.append(
    Reaction(
        sid=f"DUL2DM",
        name=f"dulaglutide peptide cleavage",
        equation=f"Cve_dul -> Cve_dm",
        pars=[
            Parameter(
                "DUL2DM_k",
                0.0006782720470435979,
                U.l_per_min,
                constant=True,
                sboTerm=SBO.FORWARD_RATE_CONSTANT,
                name=f"dulaglutide cleavage rate",
            ),
        ],
        formula=("DUL2DM_k * Cve_dul", U.mmole_per_min),  # l/min * mmole/l
        sboTerm=SBO.TRANSPORT_REACTION,
        annotations=[],
        compartment="Vve",
    )
...
# --------------------------------------------------------------------------------------------------
# Total dulaglutide metabolites
# --------------------------------------------------------------------------------------------------
_m.species.extend(
    [
        Species(
            f"Cve_dmtot",
            metaId=f"meta_Cve_dmtot",
            initialConcentration=0,
            compartment="Vve",
            substanceUnit=U.mmole,
            name=f"total dulaglutide (plasma)",
            hasOnlySubstanceUnits=False,
            sboTerm=SBO.SIMPLE_CHEMICAL,
            annotations=annotations.species["dmtot"],
        ),
    ]
)
...
_m.rules.extend([
    AssignmentRule(
        ...
        "Cve_dul + Cve_dm",
        unit=U.mM,
        name="Sum of dulaglutide and dulaglutide metabolites",
        notes="""
        Used for comparison with total plasma radioactivity.
        """
    ),
])
...
for sid, sdict in SUBSTANCES_BODY.items():
    ...
    if "SCDOSE" in sdict:
        ...
        _m.parameters.extend(
            [
                Parameter(
                    f"SCDOSE_{sid}", sdict["SCDOSE"], U.mg,
                    constant=False,
                    sboTerm=SBO.QUANTITATIVE_SYSTEMS_DESCRIPTION_PARAMETER,
                    name=f"subcutaneous dose {sid} [mg]",
                ),
                Parameter(
                    f"Ksc_{sid}",
                    0.0003284890536033426,
                    U.per_min,
                    sboTerm=SBO.QUANTITATIVE_SYSTEMS_DESCRIPTION_PARAMETER,
                    name=f"subcutaneous application rate {sid}",
                ),
            ]
        )
        _m.reactions.append(
            Reaction(
                sid=f"sc_{sid}",
                name=f"sc {sname}",
                formula=(f"Ksc_{sid}*SCDOSE_{sid}/Mr_{sid}", U.mmole_per_min),
                equation=f"-> Cve_{sid}",
                sboTerm=SBO.TRANSPORT_REACTION,
                annotations=[(BQB.IS, "NCIT:C38276")],
                compartment="Vve",
        ...
        _m.rate_rules.append(
            RateRule(
                f"SCDOSE_{sid}", f"-sc_{sid}*Mr_{sid}", U.mg_per_min
            )
        )
...
model_body.rules.append(
    AssignmentRule("glp1", "Cve_dul", unit=U.mM)
)
...
if __name__ == "__main__":
    ...
    from pkdb_models.models.bodyweight.models.model_bodyweight import model_bodyweight
    from pkdb_models.models.hba1c.models.model_hba1c import model_hba1c

    model_hba1c.parameters = [p for p in model_hba1c.parameters if p.sid != "D"]
