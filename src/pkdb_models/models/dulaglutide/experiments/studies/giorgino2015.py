...
class Giorgino2015(DulaglutideSimulationExperiment):
    """Simulation experiment of Giorgino2015.
    ...
    def datasets(self) -> Dict[str, DataSet]:
        dsets = {}
        for fig_id in ["Fig2b", "Fig2d", "Fig2f"]:
            df = load_pkdb_dataframe(f"{self.sid}_{fig_id}", data_path=self.data_path)
            for label, df_label in df.groupby("label"):
                dset = DataSet.from_df(df_label, self.ureg)
                if label.startswith("fpg_"):
    ...
    def figures(self) -> Dict[str, Figure]:
        ...
        plots[1].set_yaxis(self.label_fpg, unit=self.unit_fpg)
...
if __name__ == "__main__":
    run_experiments(Giorgino2015, output_dir=Giorgino2015.__name__)